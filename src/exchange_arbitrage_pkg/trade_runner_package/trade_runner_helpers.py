...
async def execute_coinbase_trade(coinbase_async_client, trade, debug):
    ...
    if trade.trade_type == 'check':
        result = await check_balance_coinbase(coinbase_async_client, trade.symbol)
    elif trade.trade_type == 'withdraw':
        result = await withdraw_coinbase(coinbase_async_client, trade.symbol, trade.quantity, trade.address)
    elif trade.trade_type in ['buy', 'sell']:
        result = await buy_sell_coinbase(coinbase_async_client, trade.symbol, trade.side, trade.quantity, trade.price)
    elif trade.trade_type == 'deposit':
        result = await get_deposit_address_coinbase(coinbase_async_client, trade.symbol)
    else:
        print(f"Unsupported trade type: {trade.trade_type}")
        result = None
    return trade.trade_type, result
