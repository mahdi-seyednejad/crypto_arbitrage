...
class OperationExecutor:
    ...
    async def execute_binance_trade(self, binance_async_client, trade, debug):
        ...
        if trade.trade_type == 'check':
            ...
            result = await withdraw(binance_async_client, trade.symbol, trade.quantity, trade.address,
                                    converted_network,
                                    debug=debug)
...
async def execute_trade(trade, debug):
    network_convertor_obj = BiCbNetworkConvertor(ExchangeNames.Binance, ExchangeNames.Coinbase)
    if trade.exchange_platform.name == ExchangeNames.Binance:
        binance_client = await trade.exchange_platform.create_async_client()
...
async def execute_binance_trade(binance_async_client, trade, debug):
    ...
    if trade.trade_type == 'check':
        ...
        converted_network = network_convertor_obj.convert(trade.network)
        result = await withdraw(binance_async_client, trade.symbol, trade.quantity, trade.address, converted_network,
                                debug=debug)
    elif trade.trade_type == 'buy':
        result = await buy_binance(binance_async_client, trade.symbol, trade.quantity, trade.price, debug)
    elif trade.trade_type == 'sell':
