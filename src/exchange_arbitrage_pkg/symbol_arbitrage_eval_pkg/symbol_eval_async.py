...
#         return df
#
#     async def extract_symbol_metrics(self,
#                                      df_in,
#                                      index,
#                                      row,
...
#         order_book_raw = await exchange.get_order_book(row[self.col_info.symbol_col])
...
#
#     async def extract_symbol_metrics_df(self, row, exchange: ExchangeAbstractClass, exchange_index: int):
#         exchange_name = exchange.name.value
#         order_book_raw = await exchange.get_order_book(exchange.async_obj, row[self.col_info.symbol_col])
#         order_book = remove_outliers(order_book_raw, self.hype_params.outlier_threshold)
#         order_book.reset_index(inplace=True)
#         order_book.drop(columns=['index'], inplace=True)
#
...
#         return row
#
#     async def evaluate_symbols_for_trade_df(self, df_in, exchange_list):
#         df = df_in.copy()
#         df[self.market_impact_col] = 0
#         for i, exchange in enumerate(exchange_list):
#             df = df.apply(lambda row: await self.extract_symbol_metrics_df(row, exchange, i), axis=1)
#
#         return df
#
...
#
#     async def evaluate_then_rank_best_symbols(self, df_in, exchange_list):
...
#         df_evaluated = await self.evaluate_symbols_for_trade_df(df_in, exchange_list)
