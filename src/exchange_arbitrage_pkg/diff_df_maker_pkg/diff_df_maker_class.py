...
class PriceDiffExtractor:
    ...
    async def get_latest_prices_binance(self): #ToDO: move this to Exchange object
        ...
        rename_dict = {col: (f'{self.binance_exchange.name.value}_{col}_24h' if col not in ['symbol', 'lastPrice'] else (
            self.binance_exchange.price_col if col == 'lastPrice' else col)) for col in filtered_df.columns}
        binance_df = filtered_df.rename(columns=rename_dict)

        return binance_df

    def _get_latest_prices_coinbase_at(self): #ToDO: move this to Exchange onject
        ...
        cb_price_df_not_nan.rename(columns={'id': 'symbol', 'price': self.coinbase_exchange.price_col}, inplace=True)
    ...
    async def create_differential_df(self):
        ...
        if self.debug:
            ...
            selected_cols = [self.col_info.current_time_col, self.col_info.symbol_col,
                             self.binance_exchange.price_col, self.coinbase_exchange.price_col,
