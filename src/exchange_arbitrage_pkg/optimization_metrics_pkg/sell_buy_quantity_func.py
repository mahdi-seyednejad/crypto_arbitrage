...
def calculate_max_buy_quantity(order_book, acceptable_slippage):
    """
    Calculate the maximum quantity of a crypto that can be bought without exceeding the acceptable slippage.

    :param order_book: A DataFrame with 'price' and 'volume' of sell orders (asks).
    :param acceptable_slippage: The maximum acceptable price impact in percentage (e.g., 0.5 for 0.5%).
    :return: The maximum buy quantity.
    """
    lowest_ask = order_book['price'].min()
    acceptable_price = lowest_ask * (1 + acceptable_slippage / 100)
