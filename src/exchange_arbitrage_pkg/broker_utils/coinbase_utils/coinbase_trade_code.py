...
async def buy_sell_coinbase(client, symbol, side, quantity, price=None):
    adjusted_quantity = adjust_trade_amount_coinbase(symbol=symbol, suggested_amount=quantity, price=price)
    # Assumes create_order method takes 'buy' or 'sell' as order_type, and price can be None for market orders
    return await client.create_order(order_type=side, amount=adjusted_quantity, currency=symbol, price=price)


# Function to withdraw currency to a specified address
async def withdraw_coinbase(client, symbol, amount, crypto_address):
...
async def get_deposit_address_coinbase(client, symbol):
    currency = get_base_from_pair_coinbase(symbol)
